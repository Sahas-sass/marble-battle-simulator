...
export default function Arena10() {
  ...
  // --- STATE (Scaled up to 10 Constructors) ---
  const [activeTeams, setActiveTeams] = useState(10);
  const [marbleCount, setMarbleCount] = useState(1); 
  const [teamColors, setTeamColors] = useState([
    '#00ff87', '#ff0055', '#ffeb3b', '#00d2ff', '#b700ff',
    '#ffffff', '#ff9000', '#0066ff', '#1c1c1c', '#00ffc4'
  ]);
  const [teamNames, setTeamNames] = useState([
    'Aston Martin', 'Ferrari', 'McLaren', 'Mercedes', 'Red Bull',
    'Haas', 'Alpine', 'Williams', 'Sauber', 'RB Team'
  ]);
  const [gameStatus, setGameStatus] = useState('idle');

  // --- REFS ---
  const statusRef = useRef('idle');
  const activeTeamsRef = useRef(10);
  const teamColorsRef = useRef(teamColors);
  const teamNamesRef = useRef(teamNames);
  const marbleCountRef = useRef(1);
  ...
  useEffect(() => {
    ...
    const buildHyperTrack = () => {
      ...
      // 🟩 SECTION 3: Sloped Staircase Step Chutes
      for (let j = 0; j < 5; j++) {
      ...
      // 🏁 SECTION 5: Funnel Pipeline Gates
      Composite.add(engineRef.current.world, [
      ...
      // 🔵 SECTION 6: Playful Bumper Alley (High Restitution Chaos)
      const bouncyBumpers = [];
      for (let row = 0; row < 5; row++) {
        const yPos = 6400 + (row * 160);
        bouncyBumpers.push(
          Bodies.circle(width / 2 - 120, yPos, 22, { isStatic: true, restitution: 1.6, render: { fillStyle: '#00ff87' } }),
          Bodies.circle(width / 2 + 120, yPos, 22, { isStatic: true, restitution: 1.6, render: { fillStyle: '#00d2ff' } }),
          Bodies.circle(width / 2, yPos + 80, 22, { isStatic: true, restitution: 1.6, render: { fillStyle: '#ffeb3b' } })
        );
      }
      Composite.add(engineRef.current.world, bouncyBumpers);

      // 🟣 SECTION 7: The Zig-Zag Sieve
      for (let k = 0; k < 4; k++) {
        const yPos = 7400 + (k * 220);
        const isLeft = k % 2 === 0;
        Composite.add(engineRef.current.world, [
          Bodies.rectangle(isLeft ? 160 : width - 160, yPos, 380, 20, { isStatic: true, angle: isLeft ? 0.15 : -0.15, render: { fillStyle: '#ffffff' } }),
          Bodies.rectangle(isLeft ? width - 80 : 80, yPos + 110, 220, 20, { isStatic: true, angle: isLeft ? -0.2 : 0.2, render: { fillStyle: '#b700ff' } })
        ]);
      }

      // Final Narrowing Funnel before the finish
      Composite.add(engineRef.current.world, [
        Bodies.rectangle(120, 8400, 280, 20, { isStatic: true, angle: 0.6, render: { fillStyle: '#ff0055' } }),
        Bodies.rectangle(width - 120, 8400, 280, 20, { isStatic: true, angle: -0.6, render: { fillStyle: '#ff0055' } })
      ]);
    ...
    Events.on(render, 'afterRender', () => {
      ...
      if (currentStatus === 'idle') {
        ...
        ctx.fillText('MEGA HYPER TRACK', width / 2, height / 2 - 20);
      ...
      if (currentStatus === 'countdown') {
        ...
        ctx.fillStyle = 'rgba(18, 19, 28, 0.96)';
        ctx.fillRect(0, 0, width, height);
        ctx.textAlign = 'center';
        
        if (t > 2) {
          // 📢 AUDIENCE RETENTION HOOK: Left Column for Teams, Right Column for Call to Action
          ctx.fillStyle = '#ffffff';
          ctx.font = 'bold 26px Inter';
          ctx.fillText('STARTING GRID RAMP', width / 2, 80);

          // Render 2 Columns of Teams to support up to 10 lines cleanly
          ctx.font = 'bold 15px Inter';
          for (let i = 0; i < active; i++) {
            const col = i < 5 ? 0 : 1;
            const row = i % 5;
            const xPos = col === 0 ? 60 : width / 2 + 30;
            const yPos = 160 + (row * 50);

            ctx.beginPath();
            ctx.arc(xPos, yPos - 5, 10, 0, Math.PI * 2);
            ...
            ctx.fillStyle = '#ffffff';
            ctx.textAlign = 'left';
            ctx.fillText(names[i] || `Team ${i + 1}`, xPos + 22, yPos);
          }

          // 💬 The Viral Comment Call to Action Panel
          ctx.fillStyle = 'rgba(0, 210, 255, 0.1)';
          ctx.fillRect(40, 460, width - 80, 140);
          ctx.lineWidth = 2;
          ctx.strokeStyle = '#00d2ff';
          ctx.strokeRect(40, 460, width - 80, 140);

          ctx.textAlign = 'center';
          ctx.fillStyle = '#00d2ff';
          ctx.font = '900 22px Inter';
          ctx.fillText('QUICK! LOCK IT IN!', width / 2, 510);
          ctx.fillStyle = '#ffffff';
          ctx.font = 'bold 16px Inter';
          ctx.fillText('Comment the team you support! 👇', width / 2, 555);
      ...
      if (currentStatus === 'finished') {
        ...
        ctx.fillText(`${names[winnerIndex]?.toUpperCase()}`, width / 2, height / 2 - 15);
        ctx.fillStyle = '#ffffff';
        ctx.font = 'bold 20px Inter';
        ctx.fillText(`TAKES THE CHECKERED FLAG! 🏆`, width / 2, height / 2 + 20);
  ...
  const startRecordingSequence = () => {
    ...
    recorder.onstop = () => {
      ...
      a.href = url;
      a.download = `MegaHyper-Race-${Date.now()}.webm`;
      a.click();
  ...
  const spawnRacingMarbles = () => {
    ...
    for (let t = 0; t < activeTeamsRef.current; t++) {
      for (let m = 0; m < marbleCountRef.current; m++) {
        // Stagger coordinates slightly wider horizontally to account for 10 separate team launches
        const spawnX = width / 2 + (Math.random() * 120 - 60);
        const marble = Bodies.circle(spawnX, 100 - (m * 20), 11, { 
          restitution: 0.55, 
          friction: 0.01, 
          frictionAir: 0.012, 
          label: `team_${t}`, 
  ...
  let btnText = 'Start Mega Hyper Race 🔴🏎️';
  ...
  return (
    <div className="app-container">
      <div className="sidebar" style={{ maxHeight: '100vh', overflowY: 'auto' }}>
        <h2>🏎️ Mega Hyper Race</h2>
        <div className="control-group">
          <label>Number of Constructors: {activeTeams}</label>
          <input type="range" min="2" max="10" value={activeTeams} disabled={gameStatus !== 'idle'} onChange={(e) => setActiveTeams(Number(e.target.value))} />
        </div>
        <div className="control-group">
        ...
        <div className="color-selectors" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          ...
          {Array.from({ length: activeTeams }).map((_, idx) => (
            <div className="color-picker" key={idx} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <input type="color" value={teamColors[idx]} disabled={gameStatus !== 'idle'} onChange={(e) => updateTeamColor(idx, e.target.value)} style={{ padding: '0', width: '24px', height: '24px', border: 'none', background: 'transparent', cursor: 'pointer' }} />
              <input type="text" value={teamNames[idx] || ''} placeholder={`Team ${idx + 1}`} disabled={gameStatus !== 'idle'} onChange={(e) => handleNameChange(idx, e.target.value)} style={{ flex: 1, padding: '4px 8px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', borderRadius: '4px', fontSize: '13px', outline: 'none' }} />
            </div>
          ))}
        </div>

        <button className={`btn ${gameStatus === 'idle' ? 'btn-record' : 'btn-secondary'}`} onClick={startRecordingSequence} disabled={gameStatus !== 'idle'} style={{ marginTop: '15px', width: '100%' }}>
          {btnText}
        </button>
      </div>
